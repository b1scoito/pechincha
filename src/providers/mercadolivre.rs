...
fn parse_ml_html(html: &str, _max_results: usize) -> Vec<Product> {
    ...
    for card in document.select(&card_sel).take(50) {
        if let Some(product) = parse_ml_card(&card, &title_sel, &price_sel, &img_sel, &shipping_sel) {
...
fn parse_ml_card(
    ...
    price_sel: &Selector,
    img_sel: &Selector,
    shipping_sel: &Selector,
) -> Option<Product> {
    let title_anchor = card.select(title_sel).next()?;
...
impl Provider for MercadoLivre {
    ...
    fn parse_html(&self, html: &str, max_results: usize) -> Result<Vec<Product>, ProviderError> {
        Ok(parse_ml_html(html, max_results))
    }
